  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Save FCM Token">
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-slate-500 dark:text-slate-400">User ID</label>
          <input
            readOnly
            value={userId}
            className="w-full mt-1 px-3 py-2 border rounded-md bg-gray-50 dark:bg-slate-800 dark:border-slate-700 text-slate-900 dark:text-slate-200"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-500 dark:text-slate-400">FCM Token</label>
          <textarea
            readOnly
            value={token}
            rows={3}
        ...
        <div className="text-sm text-slate-500 dark:text-slate-400">
          Route: <code className="px-1 py-0.5 bg-gray-100 dark:bg-slate-800 rounded">POST /fcm/token</code>
        </div>
        {message && (
          <div className="text-sm text-blue-600 dark:text-blue-400">{message}</div>
        )}
        <div className="flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 rounded bg-gray-200 dark:bg-slate-700 text-slate-900 dark:text-slate-100">Close</button>
          <button
            onClick={handleSave}
