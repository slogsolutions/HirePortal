...
export default function AdminUsersWithDetail() {
  ...
  const fetchUsersAndReport = async (y = year, m = month) => {
    ...
    try {
      ...
      const rRes = await api.get("/attendance/admin/report", {
        params: { start, end },
      });
      ...
      const summaries = usersData.map((u) => {
        ...
        const workingCount =
          Number(tagCounts.Working || 0) + Number(tagCounts.Holiday || 0); // holiday as working
        ...
        const pctWorking =
          daysInMonth > 0 ? (workingCount / daysInMonth) * 100 : 0;
        const pctOnLeave =
          daysInMonth > 0 ? (onLeaveCount / daysInMonth) * 100 : 0;
        const pctMissed =
          daysInMonth > 0 ? (missedCount / daysInMonth) * 100 : 0;
  ...
  const reportMap = {};
  (report || []).forEach((r) => {
    reportMap[r.userId] = r.tagCounts || {};
  });
  ...
  const startEdit = (date) => {
    setDays((prev) =>
      prev.map((d) =>
        d.date === date
          ? { ...d, tag: STATUS_OPTIONS.includes(d.tag) ? d.tag : "Working" }
          : d
      )
    );
    setEditingDay(date);
      ...
      prev.map((d) => {
        if (d.date === date) {
          ...
          if (field === "fullNote") {
            ...
            updated.displayNote =
              value.split(" ").length > 15 ? truncated + "..." : truncated;
          }
  ...
  const ChartTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div className="bg-white p-2 rounded border text-sm shadow">
        ...
        <div>
          Working: {p.WorkingCount} ({p.WorkingPct}%)
        </div>
        <div>
          On Leave: {p.OnLeaveCount} ({p.OnLeavePct}%)
        </div>
        <div>
          Missed: {p.MissedCount} ({p.MissedPct}%)
        </div>
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-2 text-sm text-gray-600">
        Note: Holidays count as <strong>Working</strong> days in the report.
      </div>
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        <button
          ...
          onClick={() => fetchUsersAndReport(year, month)}
        >
          Refresh Report
        </button>
        ...
        <div className="lg:w-1/3 space-y-4 ">
          <div className="p-3 bg-white rounded shadow-sm dark:bg-slate-800">
            <div className="flex items-center justify-between dark:bg-slate-800">
              ...
              <button
                className="text-sm px-4 py-2 rounded-xl text-indigo-600 dark:bg-indigo-800 dark:text-gray-100"
                onClick={() => fetchUsersAndReport(year, month)}
              >
                Refresh
              </button>
            ...
            {loadingUsers ? (
              <div className="text-sm text-gray-500 mt-2 dark:bg-slate-800">
                Loading users...
              </div>
            ) : (
          ...
          <div className="p-3 bg-white rounded shadow-sm dark:bg-slate-800">
            ...
            <div className="mt-3 text-xs text-gray-500">
              Bars show percentages of the month per employee (Working / On
              Leave / Missed).
            </div>
          </div>
        </div>
        ...
        <div className="lg:w-2/3 bg-white rounded shadow-sm p-4 dark:bg-slate-900">
          ...
          {!selectedUser ? (
            <div>
              <div className="flex items-center justify-between mb-3">
                <div>
                  <div className="text-lg font-semibold">
                    Today's reporting — {new Date().toISOString().slice(0, 10)}
                  </div>
                  <div className="text-sm text-gray-600">
                    Overview of everyone's reporting for today
                  </div>
                </div>
              ...
              {loadingToday ? (
                <div className="text-sm text-gray-500 dark:bg-slate-800">
                  Loading today's reporting...
                </div>
              ) : (
                <div className="space-y-3 max-h-[60vh] overflow-auto dark:bg-slate-800">
                  {todaysReport.length === 0 ? (
                    <div className="text-center text-gray-600 dark:bg-slate-800">
                      No data for today.
                    </div>
                  ) : (
                    todaysReport.map((u) => {
                      const t = u.today || {};
                      const cellClass =
                        STATUS_COLORS[t.tag] || STATUS_COLORS.Default;
                      return (
                        <div
                          key={u.userId}
                          className="p-3 rounded border bg-white flex justify-between items-start dark:bg-slate-800"
                        >
                          <div>
                            ...
                            <div className="text-sm mt-2">
                              {t.displayNote ||
                                t.note ||
                                (t.tag === "Pending" ? "No report yet" : "")}
                            </div>
                          </div>
                          <div className="text-right">
                            <div
                              className={`text-sm font-semibold px-3 py-1 rounded ${cellClass}`}
                            >
                              {t.tag}
                            </div>
                          </div>
            ...
            <>
              <div className="flex items-center justify-between mb-3 ">
                ...
                <div className="flex items-center gap-3 ">
                  ...
                  />
                  <button
                    className="text-sm text-gray-600 dark:bg-emerald-800 px-4 py-2 rounded-2xl dark:text-gray-100 "
                    onClick={() =>
                      fetchUserMonth(selectedUser._id, year, month)
                    }
                  >
                    Refresh
                  </button>
              ...
              <div className="mb-4 p-3 rounded bg-gradient-to-r from-green-50 to-blue-50 border dark:bg-slate-800">
                <div className="flex items-center justify-between ">
                  <div>
                    <div className="text-sm text-gray-600">
                      Selected Employee report for this month
                    </div>
                    <div className="text-xl font-semibold">
                      {selectedUser.name || selectedUser.email} • {year}-
                      {pad(month)}
                    </div>
                  </div>
                  <div className="text-right">
              ...
              {loadingDetail ? (
                ...
                <>
                          ...
                          <tr key={wi}>
                            {week.map((d, ci) => {
                              const cellClass = d
                                ...
                                : "bg-gray-50 text-black dark:bg-slate-300";
                              return (
                                <td
                                  key={ci}
                                  ...
                                  {d ? (
                                    <>
                                      <div className="flex justify-between items-center mb-1 px-1">
                                        ...
                                        <span
                                          className={`text-xs font-semibold px-2 py-0.5 rounded ${cellClass}`}
                                        >
                                          {d.tag}
                                        </span>
                                      </div>
                        ...
                        <PieChart>
                          <Pie
                            data={[
                              {
                                name: "Working",
                                value: userSummary.working ?? 0,
                              },
                              {
                                name: "On Leave",
                                value: userSummary.leave ?? 0,
                              },
                              {
                                name: "Missed",
                                value: userSummary.missed ?? 0,
                              },
                              {
                                name: "Holiday",
                                value: userSummary.holiday ?? 0,
                              },
                            ]}
                            dataKey="value"
                            nameKey="name"
                  ...
                  <div className="mt-6 bg-white p-2 rounded shadow-sm">
                    <h3 className="text-sm font-semibold mb-2">
                      Overall Users Performance — {year}-{pad(month)}
                    </h3>
                    ...
                    <div className="mt-2 text-xs text-gray-500">
                      Tooltip shows raw counts and percentages for each
                      employee.
                    </div>
                  </div>
                </>
