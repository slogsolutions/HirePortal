...
export default function EmployeeCalendarModern() {
  ...
  const handleDayChange = (date, field, value) => {
    setDays(prev => prev.map(d => d.date === date ? {
      ...d,
      [field]: value,
      displayNote: field === 'fullNote' ? (value.split(' ').slice(0,15).join(' ') + (value.split(' ').length>15?'...':'')) : d.displayNote
    } : d));
  };
  ...
  const handleSaveDay = async (day) => {
    try {
      ...
      const payloadEntry = { date: cur.date, tag: sendTag, note: cur.fullNote || '' };
      setDays(prev => prev.map(d => d.date === cur.date ? { ...d, tag: sendTag, fullNote: payloadEntry.note } : d));
      await api.post("/attendance/me/entries", { entries: [payloadEntry], forceEdit: forceEditMode });
      setEditingDay(null);
      await fetchMonth();
    } catch (err) {
      console.error(err);
  ...
  const handleResetDay = (date) => {
    setDays(prev => prev.map(d => d.date === date ? { ...d, tag: 'Working', fullNote: '', displayNote: '' } : d));
    setEditingDay(null);
  };
  ...
  return (
    <div className="p-6 font-sans">
      <h1 className="text-2xl font-bold mb-4">Employee Calendar {forceEditMode && '(Force Edit ON)'}</h1>
      ...
      <div className="flex gap-4 mb-4 items-center">
        <input type="month" value={`${year}-${month.toString().padStart(2,'0')}`} onChange={e=>{ const [y,m]=e.target.value.split('-'); setYear(Number(y)); setMonth(Number(m)); }} className="border rounded px-2 py-1 text-sm" />
      </div>
      ...
      <div className="grid grid-cols-7 gap-2">
        {weeks.flat().map((d, idx) => (
          <div
            key={idx}
            className={`relative rounded-lg min-h-[100px] p-2 flex flex-col items-start justify-start ${d ? (STATUS_COLORS[d.tag] || STATUS_COLORS.Default) : 'bg-gray-50'}`}
            onMouseEnter={() => d && setHoveredDay(d.date)}
            onMouseLeave={() => setHoveredDay(null)}
          >
            {d && (
              <>
                <div className="w-full flex justify-center pointer-events-none"><span className="font-bold text-base">{d.day}</span></div>

                {/* Show full content on hover or if editing */}
                {(hoveredDay===d.date || editingDay===d.date) && (
                  <div className="w-full bg-white rounded-lg p-2 mt-1 border shadow flex flex-col break-words whitespace-pre-wrap">
                    ...
                    <div className="flex justify-end gap-2">
                      {editingDay===d.date ? (
                        <>
                          ...
                          <button className="px-2 py-1 bg-green-600 text-white rounded" onClick={()=>handleSaveDay(d)}><CheckIcon className="h-4 w-4"/></button>
                          <button className="px-2 py-1 bg-red-600 text-white rounded" onClick={()=>handleResetDay(d.date)}><XMarkIcon className="h-4 w-4"/></button>
                        </>
                      ) : (
                        d.editable && <PencilIcon className="h-5 w-5 cursor-pointer text-gray-800" onClick={()=>startEdit(d.date)}/>
                      )}
                    </div>
                  </div>
                )}
