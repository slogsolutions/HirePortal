...
export default function CandidatesPage() {
    ...
    return (
      <form onSubmit={submit} className="space-y-4 max-h-[80vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-4">
          <div className="w-20 h-20 rounded-full overflow-hidden border bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
    ...
    <div key={c._id} className="flex items-center gap-4 p-4 md:p-6 border-b last:border-b-0 bg-white dark:bg-slate-900">
      <div className="flex-shrink-0">
        <button onClick={() => setAvatarPreview(c.photoUrl || null)} className="w-14 h-14 rounded-full overflow-hidden border">
          {c.photoUrl ? <img src={c.photoUrl} alt={c.firstName} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center bg-slate-100 dark:bg-slate-800">{(c.firstName?.[0] || "?").toUpperCase()}</div>}
        </button>
      </div>
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-lg shadow-md overflow-hidden divide-y divide-gray-100 dark:divide-slate-800">
